...
<!--ribbon at the top of the page ------>
	...
	<h3>Enter the product code of the product that has to be deleted.</h3>
...
<?php
	...
	$query="DELETE FROM BBproducts WHERE code=$code";
	$dbh=mysql_connect($host,$db_user,$db_password);
	mysql_select_db($database);
	mysql_query($query,$dbh);
	$d = mysql_affected_rows();
	if ($d == 0){
		echo "<h2>The product with the entered product code is not in the database!</h2>";
	}
	else {	
	echo "<h2>The product was deleted successfully!</h2>";
	}
?>
<?php
 $host="db-mysql.zenit";
